...
export class CustomerAccountsComponent implements OnInit {
      ...
      this.accountService
        ...
        .subscribe({
          ...
          error: (err) => {
            this.notificationService.showError(
              'Error',
              'Failed to create saving account'
            );
            this.loading = false;
          },
        });
  ...
  isSavingAccount(account: BankAccount): account is SavingBankAccount {
    return account.type === 'SAVING_ACCOUNT';
  }

  isCurrentAccount(account: BankAccount): account is CurrentBankAccount {
    return account.type === 'CURRENT_ACCOUNT';
  }

  // Form getters
  get initialBalance() {
    return this.newAccountForm.get('initialBalance');
  }
  get accountTypeValue() {
    return this.newAccountForm.get('accountTypeValue');
  }
}
