...
import {
  ...
  StatusTransition,
} from '../../../core/services/account-status.service';
...
export class AccountsListComponent implements OnInit {
  @ViewChild('statusConfirmModal')
  statusConfirmModal!: ConfirmationModalComponent;
  ...
  onStatusChangeConfirmed(): void {
    ...
    this.accountService.changeAccountStatus(account.id, newStatus).subscribe({
      ...
      error: (error) => {
        ...
        const errorMessage =
          error.error?.message ||
          `Failed to change account status to ${newStatus}`;
